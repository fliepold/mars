...
int xio_throttle_start = 0;
EXPORT_SYMBOL_GPL(xio_throttle_start);

int xio_throttle_end = 99;
...
int compute_emergency_mode(void)
{
	...
	if (xio_throttle_start > 0 &&
		...
		if (percent < xio_throttle_start) {
			if_throttle_start_size = 0;
		} else if (percent >= xio_throttle_end) {
			if_throttle_start_size = 1;
		} else {
			if_throttle_start_size = (xio_throttle_end - percent) * 1024 / (xio_throttle_end - xio_throttle_start) + 1;
		}
	}
...
typedef int (*light_worker_fn)(void *buf, struct xio_dent *dent);
...
struct mars_rotate {
	...
	struct xio_dent *replay_link;
	struct xio_brick *bio_brick;
	struct xio_dent *aio_dent;
	struct aio_brick *aio_brick;
	struct xio_info aio_info;
	struct trans_logger_brick *trans_brick;
	struct xio_dent *first_log;
	struct xio_dent *relevant_log;
	struct xio_brick *relevant_brick;
	struct xio_dent *next_relevant_log;
	struct xio_brick *next_relevant_brick;
	struct xio_dent *next_next_relevant_log;
	struct xio_dent *prev_log;
	struct xio_dent *next_log;
	struct xio_dent *syncstatus_dent;
	...
	struct xio_limiter file_limiter;
...
static int _parse_args(struct xio_dent *dent, char *str, int count)
{
	int i;
	int status = -EINVAL;
	if (!str)
...
static
bool _check_switch(struct xio_global *global, const char *path)
{
	int res = false;
	struct xio_dent *allow_dent;

	allow_dent = mars_find_dent(global, path);
	if (!allow_dent || !allow_dent->link_val)
		goto done;
	sscanf(allow_dent->link_val, "%d", &res);
	XIO_DBG("'%s' -> %d\n", path, res);
...
static
bool _check_allow(struct xio_global *global, struct xio_dent *parent, const char *name)
{
	int res = false;
...
int _update_link_when_necessary(struct mars_rotate *rot, const char *type, const char *old, const char *new)
{
	...
	if (unlikely(status < 0)) {
		XIO_ERR_TO(rot->log_say, "cannot create %s symlink '%s' -> '%s' status = %d\n", type, old, new, status);
	} else {
		res = 1;
	...
	if (likely(inf->inf_sequence > 1)) {
		if (unlikely((inf->inf_sequence < rot->inf_prev_sequence ||
			...
			if (likely(skip_nr != inf->inf_sequence)) {
				XIO_ERR_TO(rot->log_say, "SKIP in sequence numbers detected: %d != %d + 1\n", inf->inf_sequence, rot->inf_prev_sequence);
				goto out;
			}
...
void _update_info(struct trans_logger_info *inf)
{
	...
	XIO_DBG("inf = %p '%s' seq = %d min_pos = %lld max_pos = %lld log_pos = %lld is_applying = %d is_logging = %d\n",
		 inf,
	...
	if (unlikely(rot->infs_is_dirty[hash])) {
		if (unlikely(rot->infs[hash].inf_sequence != inf->inf_sequence)) {
			XIO_ERR_TO(rot->log_say, "buffer %d: sequence trash %d -> %d. is the mar_light thread hanging?\n", hash, rot->infs[hash].inf_sequence, inf->inf_sequence);
		} else {
...
void write_info_links(struct mars_rotate *rot)
{
	...
	for (;;) {
		...
		XIO_DBG("seq = %d min_pos = %lld max_pos = %lld log_pos = %lld is_applying = %d is_logging = %d\n",
			 inf.inf_sequence,
			 inf.inf_min_pos,
			 inf.inf_max_pos,
			 inf.inf_log_pos,
...
static
void _show_primary(struct mars_rotate *rot, struct xio_dent *parent)
{
	int status;
	if (!rot || !parent) {
		return;
...
static
void _show_status_all(struct xio_global *global)
{
...
{
	struct xio_global *global = rot->global;
...
static
int check_logfile(const char *peer, struct xio_dent *remote_dent, struct xio_dent *local_dent, struct xio_dent *parent, loff_t dst_size)
{
	...
	if (!rot->copy_path) {
		XIO_WRN("parent has no copy_path\n");
		status = -EINVAL;
		goto done;
	}
...
static
int run_bone(struct mars_peerinfo *peer, struct xio_dent *remote_dent)
{
	int status = 0;
	...
	if (stat_ok) {
		...
		if (remote_dent->stat_val.uid != local_stat.uid && update_ctime) {
			mars_lchown(remote_dent->d_path, remote_dent->stat_val.uid);
			run_trigger = true;
		}
	}
	...
	if (S_ISDIR(remote_dent->stat_val.mode)) {
		...
		if (!stat_ok || update_mtime) {
			status = mars_symlink(remote_dent->link_val, remote_dent->d_path, &remote_dent->stat_val.mtime, remote_dent->stat_val.uid);
			run_trigger = true;
		}
		...
		if (likely(parent_path)) {
			struct xio_dent *parent = mars_find_dent(peer->global, parent_path);
			struct xio_dent *local_dent = mars_find_dent(peer->global, remote_dent->d_path);
			if (unlikely(!parent)) {
...
int run_bones(struct mars_peerinfo *peer)
{
	...
	for (tmp = tmp_list.next; tmp != &tmp_list; tmp = tmp->next) {
		struct xio_dent *remote_dent = container_of(tmp, struct xio_dent, dent_link);
...
int peer_thread(void *data)
{
	...
	while (!brick_thread_should_stop()) {
		...
		if (!xio_socket_is_alive(&peer->socket)) {
			...
			if (unlikely(status < 0)) {
				XIO_INF("no connection to mars module on '%s'\n", real_peer);
				brick_msleep(2000);
				continue;
			}
			do_kill = true;
		...
		if (!brick_thread_should_stop()) {
			...
			wait_event_interruptible_timeout(remote_event,
							 ...
							 (xio_global && xio_global->main_trigger),
							 pause_time * HZ);
		}
	}
...
void _make_alive(void)
{
	...
	_make_alivelink("alive", xio_global && xio_global->global_power.button ? 1 : 0);
...
static int _kill_peer(void *buf, struct xio_dent *dent)
{
	LIST_HEAD(tmp_list);
	struct xio_global *global = buf;
...
static int _make_peer(struct xio_global *global, struct xio_dent *dent, char *path)
{
	static int serial = 0;
	...
	if (!global || !global->global_power.button || !dent || !dent->link_val || !dent->d_parent || !(parent_path = dent->d_parent->d_path)) {
		XIO_DBG("cannot work\n");
		return 0;
	}
	mypeer = dent->d_rest;
...
static int kill_scan(void *buf, struct xio_dent *dent)
{
	return _kill_peer(buf, dent);
}

static int make_scan(void *buf, struct xio_dent *dent)
{
...
static
int kill_any(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	struct list_head *tmp;
...
bool is_switchover_possible(struct mars_rotate *rot, const char *old_log_path, const char *new_log_path, int replay_tolerance, bool skip_new)
{
	...
	if (unlikely(new_log_seq != old_log_seq + 1)) {
		XIO_ERR_TO(rot->log_say, "logfile sequence numbers are not contiguous (%d != %d + 1), old_log_path='%s' new_log_path='%s'\n", new_log_seq, old_log_seq, old_log_path, new_log_path);
		goto done;
	}
	...
	if (!skip_new) {
		...
		if (unlikely(!new_versionlink)) {
			XIO_INF_TO(rot->log_say, "new versionlink '%s' does not yet exist, we must wait for it.\n", SAFE_STR(new_versionlink_path));
			goto done;
		}
	}
	...
	if (unlikely(strcmp(own_versionlink, old_versionlink))) {
		XIO_INF_TO(rot->log_say, "old logfile is not yet completeley transferred, own_versionlink '%s' -> '%s' != old_versionlink '%s' -> '%s'\n", own_versionlink_path, own_versionlink, old_versionlink_path, old_versionlink);
		goto done;
	}
	...
	if (unlikely(!own_versionlink[own_v_offset++])) {
		XIO_ERR_TO(rot->log_say, "own version link '%s' -> '%s' is malformed\n", own_versionlink_path, own_versionlink);
		goto done;
	}
	...
	if (unlikely(own_r_len != own_v_len ||
		...
		XIO_ERR_TO(rot->log_say, "internal problem: logfile name mismatch between '%s' and '%s'\n", own_replaylink, own_versionlink);
		goto done;
	}
	if (unlikely(!own_replaylink[own_r_len])) {
		XIO_ERR_TO(rot->log_say, "own replay link '%s' -> '%s' is malformed\n", own_replaylink_path, own_replaylink);
		goto done;
	}
	...
	if (unlikely(own_r_len > own_v_len || own_r_len + replay_tolerance < own_v_len)) {
		XIO_INF_TO(rot->log_say, "log replay is not yet finished: '%s' and '%s' are reporting different positions.\n", own_replaylink, own_versionlink);
		goto done;
	}
 ...
 */
static
int make_log_init(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	struct xio_dent *parent = dent->d_parent;
	struct xio_brick *bio_brick;
	...
	if (!rot) {
		...
		if (unlikely(!copy_path)) {
			XIO_ERR("cannot create copy_path\n");
			brick_mem_free(rot);
			status = -ENOMEM;
			goto done;
		}
	...
	if (dent->link_val)
		sscanf(dent->link_val, "%lld", &rot->dev_size);
	if (!rot->parent_path)
 ...
 */
static
int make_log_step(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	struct xio_dent *parent = dent->d_parent;
	struct mars_rotate *rot;
	...
	if (prev_log && prev_log->d_serial + 1 != dent->d_serial) {
		XIO_WRN_TO(rot->log_say, "transaction logs are not consecutive at '%s' (%d ~> %d)\n", dent->d_path, prev_log->d_serial, dent->d_serial);
	}
...
{
	struct xio_dent *dent = rot->relevant_log;
	struct xio_dent *parent;
	struct xio_global *global = NULL;
	int status = 0;
	...
	if (sscanf(rot->replay_link->d_argv[0], "log-%d", log_nr) != 1) {
		XIO_ERR_TO(rot->log_say, "replay link has malformed logfile number '%s'\n", rot->replay_link->d_argv[0]);
		goto done;
	}
	if (sscanf(rot->replay_link->d_argv[1], "%lld", oldpos_start) != 1) {
		XIO_ERR_TO(rot->log_say, "replay link has bad start position argument '%s'\n", rot->replay_link->d_argv[1]);
		goto done;
	}
	if (sscanf(rot->replay_link->d_argv[2], "%lld", oldpos_end) != 1) {
		XIO_ERR_TO(rot->log_say, "replay link has bad end position argument '%s'\n", rot->replay_link->d_argv[2]);
		goto done;
	}
	...
	*newpos = rot->aio_info.current_size;
	...
	if (unlikely(rot->aio_info.current_size < *oldpos_start)) {
		XIO_ERR_TO(rot->log_say, "oops, bad replay position attempted at logfile '%s' (file length %lld should never be smaller than requested position %lld, is your filesystem corrupted?) => please repair this by hand\n", rot->aio_dent->d_path, rot->aio_info.current_size, *oldpos_start);
		status = -EBADF;
		goto done;
	}
	...
	if (rot->aio_info.current_size > *oldpos_start) {
		if (rot->aio_info.current_size - *oldpos_start < REPLAY_TOLERANCE &&
			...
			XIO_INF_TO(rot->log_say, "TOLERANCE: transaction log '%s' is treated as fully applied\n", rot->aio_dent->d_path);
			status = 1;
		} else {
			XIO_INF_TO(rot->log_say, "transaction log replay is necessary on '%s' from %lld to %lld (dirty region ends at %lld)\n", rot->aio_dent->d_path, *oldpos_start, rot->aio_info.current_size, *oldpos_end);
			status = 2;
		}
	} else if (rot->next_relevant_log) {
		XIO_INF_TO(rot->log_say, "transaction log '%s' is already applied, and the next one is available for switching\n", rot->aio_dent->d_path);
		status = 1;
	} else if (rot->todo_primary) {
		if (rot->aio_info.current_size > 0 || strcmp(dent->d_rest, my_id()) != 0) {
			XIO_INF_TO(rot->log_say, "transaction log '%s' is already applied (would be usable for appending at position %lld, but a fresh logfile will be used for safety reasons)\n", rot->aio_dent->d_path, *oldpos_end);
			status = 1;
		} else {
			XIO_INF_TO(rot->log_say, "empty transaction log '%s' is usable for me as a primary node\n", rot->aio_dent->d_path);
			status = 3;
		}
	} else {
...
int _make_logging_status(struct mars_rotate *rot)
{
	...
	if (unlikely(start_pos < 0 || dirty_pos < start_pos || end_pos < dirty_pos)) {
		XIO_ERR_TO(rot->log_say, "replay symlink has implausible values: start_pos = %lld dirty_pos = %lld end_pos = %lld\n", start_pos, dirty_pos, end_pos);
	}
	/* Relevant or not?
	 */
	switch (status) {
	...
	case 1: /* Relevant, and transaction replay already finished.
		...
		if (!trans_brick->power.button && !trans_brick->power.led_on && trans_brick->power.led_off) {
			if (rot->next_relevant_log) {
				...
				XIO_DBG("check switchover from '%s' to '%s' (size = %lld, next_next = %p, skip_new = %d, replay_tolerance = %d)\n", dent->d_path, rot->next_relevant_log->d_path, rot->next_relevant_log->stat_val.size, rot->next_next_relevant_log, skip_new, replay_tolerance);
				if (is_switchover_possible(rot, dent->d_path, rot->next_relevant_log->d_path, replay_tolerance, skip_new)) {
					XIO_INF_TO(rot->log_say, "start switchover from transaction log '%s' to '%s'\n", dent->d_path, rot->next_relevant_log->d_path);
					_make_new_replaylink(rot, rot->next_relevant_log->d_rest, rot->next_relevant_log->d_serial, rot->next_relevant_log->stat_val.size);
				}
			} else if (rot->todo_primary) {
				if (dent->d_serial > log_nr)
					log_nr = dent->d_serial;
				XIO_INF_TO(rot->log_say, "preparing new transaction log, number moves from %d to %d\n", dent->d_serial, log_nr + 1);
	...
	case 2: // relevant for transaction replay
		XIO_INF_TO(rot->log_say, "replaying transaction log '%s' from position %lld to %lld\n", dent->d_path, start_pos, end_pos);
		rot->replay_mode = true;
...
static
void _init_trans_input(struct trans_logger_input *trans_input, struct xio_dent *log_dent, struct mars_rotate *rot)
{
...
void _rotate_trans(struct mars_rotate *rot)
{
	...
	if (log_nr == trans_brick->new_input_nr &&
		...
		if (unlikely(!rot->next_relevant_brick)) {
			XIO_ERR_TO(rot->log_say, "could not open next transaction log '%s'\n", rot->next_relevant_log->d_path);
			goto done;
		}
		...
		XIO_INF_TO(rot->log_say, "started logrotate switchover from '%s' to '%s'\n", rot->relevant_log->d_path, rot->next_relevant_log->d_path);
	}
done: ;
}
...
static
int make_log_finalize(struct xio_global *global, struct xio_dent *dent)
{
	struct xio_dent *parent = dent->d_parent;
	struct mars_rotate *rot;
	struct trans_logger_brick *trans_brick;
	...
	if (IS_JAMMED()) {
		...
		if (!trans_logger_resume) {
			XIO_INF_TO(rot->log_say, "emergency mode on %s could be turned off now, but /proc/sys/mars/logger_resume inhibits it.\n", rot->parent_path);
		} else {
	...
	if (trans_brick->cease_logging | trans_brick->stopped_logging) {
		XIO_ERR_TO(rot->log_say, "EMERGENCY MODE on %s: stopped transaction logging, and created a hole in the logfile sequence nubers.\n", rot->parent_path);
	...
	if (IS_EMERGENCY_PRIMARY() || (!rot->todo_primary && IS_EMERGENCY_SECONDARY())) {
		...
		if (rot->first_log && rot->first_log != rot->relevant_log) {
			XIO_WRN_TO(rot->log_say, "EMERGENCY: ruthlessly freeing old logfile '%s', don't cry on any ramifications.\n", rot->first_log->d_path);
		...
		XIO_WRN_TO(rot->log_say, "EMERGENCY: the space on /mars/ is becoming low. Stopping all fetches of logfiles for secondary resources.\n");
	}

	if (trans_brick->replay_mode) {
		if (trans_brick->replay_code > 0) {
			XIO_INF_TO(rot->log_say, "logfile apply ended successfully\n");
		} else if (trans_brick->replay_code < 0) {
			XIO_ERR_TO(rot->log_say, "logfile apply stopped with error = %d\n", trans_brick->replay_code);
		}
	}
	...
	XIO_DBG("copy_path = '%s' copy_brick = %p\n", rot->copy_path, copy_brick);
	if (copy_brick &&
	    (copy_brick->power.led_off ||
	     !global->global_power.button ||
	     (copy_brick->copy_last == copy_brick->copy_end &&
	      rot->copy_next_is_available > 0))) {
		status = xio_kill_brick((void*)copy_brick);
		if (status < 0) {
			XIO_ERR("could not kill copy_brick, status = %d\n", status);
			goto done;
		}
		copy_brick = NULL;
		local_trigger();
...
static
int make_primary(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	struct xio_dent *parent;
	struct mars_rotate *rot;
...
static
int make_bio(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	struct mars_rotate *rot;
...
static int make_replay(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	struct xio_dent *parent = dent->d_parent;
	int status = 0;
...
static
int make_dev(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	struct xio_dent *parent = dent->d_parent;
...
static
int kill_dev(void *buf, struct xio_dent *dent)
{
	struct xio_dent *parent = dent->d_parent;
...
static int make_sync(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	struct mars_rotate *rot;
	loff_t start_pos = 0;
	loff_t end_pos = 0;
	struct xio_dent *size_dent;
	struct xio_dent *connect_dent;
	char *peer;
	struct copy_brick *copy = NULL;
	char *tmp = NULL;
	const char *switch_path = NULL;
	 ...
	 */
	status = sscanf(dent->link_val, "%lld", &start_pos);
	if (status != 1) {
	...
	status = sscanf(size_dent->link_val, "%lld", &end_pos);
	if (status != 1) {
	...
	if (!connect_dent || !connect_dent->link_val) {
		XIO_WRN("cannot determine peer, symlink '%s' is missing => assuming that I am STANDALONE\n", tmp);
		status = 0;
		goto done;
	}
	peer = connect_dent->link_val;
...
static int prepare_delete(void *buf, struct xio_dent *dent)
{
	struct kstat stat;
	struct xio_global *global = buf;
	struct xio_dent *target;
	struct xio_dent *response;
	const char *marker_path = NULL;
	...
	if (brick && unlikely(brick->nr_outputs > 0 && brick->outputs[0] && brick->outputs[0]->nr_connected)) {
		XIO_WRN("target '%s' cannot be deleted, its brick '%s' in use\n", dent->link_val, SAFE_STR(brick->brick_name));
		goto done;
	}

	status = -EAGAIN;
	target = _mars_find_dent(global, dent->link_val);
	if (target) {
		status = mars_unlink(dent->link_val);
		target->d_killme = true;
		XIO_DBG("target '%s' deleted (status = %d) and marked for removal\n", dent->link_val, status);
	...
	if (!response_path) {
		XIO_ERR("cannot build response path for '%s'\n", dent->link_val);
		goto done;
	}
	response = _mars_find_dent(global, response_path);
	if (response && response->link_val) {
		sscanf(response->link_val, "%d", &max_serial);
	}
...
static int check_deleted(void *buf, struct xio_dent *dent)
{
	struct xio_global *global = buf;
	int serial = 0;
	int status;
	...
	status = sscanf(dent->link_val, "%d", &serial);
 ...
 */
static
int _checker(struct xio_dent *parent, const char *_name, int namlen, unsigned int d_type, int *prefix, int *serial, bool *use_channel, bool external_mode)
{
	int class;
	int status = -2;
#ifdef XIO_DEBUGGING
...
static
int light_checker(struct xio_dent *parent, const char *_name, int namlen, unsigned int d_type, int *prefix, int *serial, bool *use_channel)
{
	return _checker(parent, _name, namlen, d_type, prefix, serial, use_channel, false);
}

int external_checker(struct xio_dent *parent, const char *_name, int namlen, unsigned int d_type, int *prefix, int *serial, bool *use_channel)
{
 ...
 */
static int light_worker(struct xio_global *global, struct xio_dent *dent, bool prepare, bool direction)
{
	light_worker_fn worker;
	int class = dent->d_class;

	if (class < 0 || class >= sizeof(light_classes)/sizeof(struct light_class)) {
		XIO_ERR_ONCE(dent, "bad internal class %d of '%s'\n", class, dent->d_path);
		return -EINVAL;
	}
	switch (light_classes[class].cl_type) {
	case 'd':
		if (!S_ISDIR(dent->stat_val.mode)) {
			XIO_ERR_ONCE(dent, "'%s' should be a directory, but is something else\n", dent->d_path);
			return -EINVAL;
		}
		break;
	case 'f':
		if (!S_ISREG(dent->stat_val.mode)) {
			XIO_ERR_ONCE(dent, "'%s' should be a regular file, but is something else\n", dent->d_path);
			return -EINVAL;
		}
		break;
	case 'F':
		if (!S_ISREG(dent->stat_val.mode) && !S_ISLNK(dent->stat_val.mode)) {
			XIO_ERR_ONCE(dent, "'%s' should be a regular file or a symlink, but is something else\n", dent->d_path);
			return -EINVAL;
		}
		break;
	case 'l':
		if (!S_ISLNK(dent->stat_val.mode)) {
			XIO_ERR_ONCE(dent, "'%s' should be a symlink, but is something else\n", dent->d_path);
			return -EINVAL;
		}
		break;
	}
	if (likely(class > CL_ROOT)) {
		...
		if (father == CL_ROOT) {
			if (unlikely(dent->d_parent)) {
				XIO_ERR_ONCE(dent, "'%s' class %d is not at the root of the hierarchy\n", dent->d_path, class);
				return -EINVAL;
			}
		} else if (unlikely(!dent->d_parent || dent->d_parent->d_class != father)) {
			XIO_ERR_ONCE(dent, "last component '%s' from '%s' is at the wrong position in the hierarchy (class = %d, parent_class = %d, parent = '%s')\n", dent->d_name, dent->d_path, father, dent->d_parent ? dent->d_parent->d_class : -9999, dent->d_parent ? dent->d_parent->d_path : "");
			return -EINVAL;
		}
	}
	if (prepare) {
...
static struct xio_global _global = {
...
static int light_thread(void *data)
{
	...
	while (_global.global_power.button || !list_empty(&_global.brick_anchor)) {
		...
		status = xio_dent_work(&_global, "/mars", sizeof(struct xio_dent), light_checker, light_worker, &_global, 3);
		...
		if (!_global.global_power.button) {
			status = xio_kill_brick_when_possible(&_global, &_global.brick_anchor, false, (void*)&copy_brick_type, true);
		...
		status = xio_kill_brick_when_possible(&_global, &_global.brick_anchor, false, (void*)&client_brick_type, true);
		XIO_DBG("kill client bricks (when possible) = %d\n", status);
		status = xio_kill_brick_when_possible(&_global, &_global.brick_anchor, false, (void*)&aio_brick_type, true);
		XIO_DBG("kill aio    bricks (when possible) = %d\n", status);
#ifdef CONFIG_MARS_DEBUG
		status = xio_kill_brick_when_possible(&_global, &_global.brick_anchor, false, (void*)&sio_brick_type, true);
		XIO_DBG("kill sio    bricks (when possible) = %d\n", status);
#endif
		status = xio_kill_brick_when_possible(&_global, &_global.brick_anchor, false, (void*)&bio_brick_type, true);
...
char *_xio_info(void)
{
	...
	down_read(&xio_global->brick_mutex);
	for (tmp = xio_global->brick_anchor.next; tmp != &xio_global->brick_anchor; tmp = tmp->next) {
		struct xio_brick *test;
		brick_count++;
		test = container_of(tmp, struct xio_brick, global_brick_link);
		pos += snprintf(
	...
	up_read(&xio_global->brick_mutex);
