...
const struct meta xio_dent_meta[] = {
	META_INI(d_name,    struct xio_dent, FIELD_STRING),
	META_INI(d_rest,    struct xio_dent, FIELD_STRING),
	META_INI(d_path,    struct xio_dent, FIELD_STRING),
	META_INI(d_namelen, struct xio_dent, FIELD_INT),
	META_INI(d_pathlen, struct xio_dent, FIELD_INT),
	META_INI(d_type,    struct xio_dent, FIELD_UINT),
	META_INI(d_class,   struct xio_dent, FIELD_INT),
	META_INI(d_serial,  struct xio_dent, FIELD_INT),
	META_INI_SUB(stat_val,struct xio_dent, mars_kstat_meta),
	META_INI(link_val,    struct xio_dent, FIELD_STRING),
	META_INI(d_args,    struct xio_dent, FIELD_STRING),
	META_INI(d_argv[0], struct xio_dent, FIELD_STRING),
	META_INI(d_argv[1], struct xio_dent, FIELD_STRING),
	META_INI(d_argv[2], struct xio_dent, FIELD_STRING),
	META_INI(d_argv[3], struct xio_dent, FIELD_STRING),
	{}
};
EXPORT_SYMBOL_GPL(xio_dent_meta);
...
void bind_to_dent(struct xio_dent *dent, struct say_channel **ch)
{
	if (!dent) {
		if (*ch) {
	...
	if (unlikely(!dent->d_say_channel)) {
		struct xio_dent *test = dent->d_parent;
		for (;;) {
			if (!test) {
...
struct xio_global *xio_global = NULL;
EXPORT_SYMBOL_GPL(xio_global);

static
void __local_trigger(void)
{
	if (xio_global) {
		xio_global->main_trigger = true;
		wake_up_interruptible_all(&xio_global->main_event);
	}
}
...
static
int get_inode(char *newpath, struct xio_dent *dent)
{
	mm_segment_t oldfs;
	int status;
	struct kstat tmp = {};
...
int xio_dent_work(struct xio_global *global, char *dirname, int allocsize, xio_dent_checker_fn checker, xio_dent_worker_fn worker, void *buf, int maxdepth)
{
...
restart:
	...
	for (tmp = global->dent_anchor.next; tmp != &global->dent_anchor; tmp = tmp->next) {
		struct xio_dent *dent = container_of(tmp, struct xio_dent, dent_link);
...
EXPORT_SYMBOL_GPL(xio_dent_work);

struct xio_dent *_mars_find_dent(struct xio_global *global, const char *path)
{
	struct xio_dent *res = NULL;
	struct list_head *tmp;
	...
	for (tmp = global->dent_anchor.next; tmp != &global->dent_anchor; tmp = tmp->next) {
		struct xio_dent *tmp_dent = container_of(tmp, struct xio_dent, dent_link);
...
struct xio_dent *mars_find_dent(struct xio_global *global, const char *path)
{
	struct xio_dent *res;
	if (!global)
		return NULL;
...
int mars_find_dent_all(struct xio_global *global, char *prefix, struct xio_dent ***table)
{
...
void xio_kill_dent(struct xio_dent *dent)
{
	dent->d_killme = true;
...
void xio_free_dent(struct xio_dent *dent)
{
	int i;
...
void xio_free_dent_all(struct xio_global *global, struct list_head *anchor)
{
	LIST_HEAD(tmp_list);
	if (global)
		down_write(&global->dent_mutex);
	...
	while (!list_empty(&tmp_list)) {
		struct xio_dent *dent;
		dent = container_of(tmp_list.prev, struct xio_dent, dent_link);
...
struct xio_brick *mars_find_brick(struct xio_global *global, const void *brick_type, const char *path)
{
	struct list_head *tmp;
...
int xio_free_brick(struct xio_brick *brick)
{
	...
	for (;;) {
		...
		if (maxsleep > 0) {
			XIO_WRN("MEMLEAK: brick '%s' has %d aios allocated (total = %d, maxsleep = %d)\n", brick->brick_path, count, atomic_read(&brick->aio_object_layout.total_alloc_count), maxsleep);
		} else {
			XIO_ERR("MEMLEAK: brick '%s' has %d aios allocated (total = %d)\n", brick->brick_path, count, atomic_read(&brick->aio_object_layout.total_alloc_count));
			break;
		}
...
struct xio_brick *xio_make_brick(struct xio_global *global, struct xio_dent *belongs, const void *_brick_type, const char *path, const char *name)
{
...
int xio_kill_brick_all(struct xio_global *global, struct list_head *anchor, bool use_dent_link)
{
	int status = 0;
...
int xio_kill_brick_when_possible(struct xio_global *global, struct list_head *anchor, bool use_dent_link, const struct xio_brick_type *type, bool even_on)
{
	int return_status = 0;
	struct list_head *tmp;
...
restart:
	...
	for (tmp = anchor->next; tmp != anchor; tmp = tmp->next) {
		...
		/* Workaround FIXME:
		 * only kill bricks which have not been touched during the current xio_dent_work() round.
...
struct xio_brick *path_find_brick(struct xio_global *global, const void *brick_type, const char *fmt, ...)
{
	va_list args;
	char *fullpath;
	struct xio_brick *res;
...
void xio_set_power_led_on(struct xio_brick *brick, bool val)
{
	bool oldval = brick->power.led_on;
	if (val != oldval) {
		//XIO_DBG("brick '%s' type '%s' led_on %d -> %d\n", brick->brick_path, brick->type->type_name, oldval, val);
		set_led_on(&brick->power, val);
		local_trigger();
	}
}
EXPORT_SYMBOL_GPL(xio_set_power_led_on);

void xio_set_power_led_off(struct xio_brick *brick, bool val)
{
	bool oldval = brick->power.led_off;
	if (val != oldval) {
		//XIO_DBG("brick '%s' type '%s' led_off %d -> %d\n", brick->brick_path, brick->type->type_name, oldval, val);
		set_led_off(&brick->power, val);
		local_trigger();
	}
}
EXPORT_SYMBOL_GPL(xio_set_power_led_off);
