...
int xio_max_loadavg = 0;
EXPORT_SYMBOL_GPL(xio_max_loadavg);
#endif
...
ctl_table mars_table[] = {
	...
	INT_ENTRY("write_throttle_start", xio_throttle_start,	  0600),
	INT_ENTRY("write_throttle_end",   xio_throttle_end,	  0600),
	INT_ENTRY("write_throttle_size_limit", if_throttle_start_size, 0400),
	LIMITER_ENTRIES(&if_throttle,	  "write_throttle",	  "kb"),
#ifdef CONFIG_MARS_LOADAVG_LIMIT
	INT_ENTRY("loadavg_limit",	  xio_max_loadavg,	  0600),
#endif
...
void __exit exit_xio_proc(void)
{
	XIO_INF("exit_proc()\n");
	if (header) {
