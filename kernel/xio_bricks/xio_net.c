...
{
	if (status < 0) {
		XIO_WRN("cannot set socket option, status = %d\n", status);
	}
}
...
int mars_proto_exchange(struct xio_socket *msock, const char *msg)
{
	int status;
...
static
int _desc_send_item(struct xio_socket *msock, const void *data, const struct xio_desc_cache *mc, int index, bool cork)
{
