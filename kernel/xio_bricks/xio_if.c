...
#define _SET_AIO_FLAGS(aio)						\
	{								\
		if (flush)						\
			aio->io_flags |= AIO_FLUSH;			\
		if (meta)						\
			aio->io_flags &= ~AIO_PERF_NOMETA;		\
		if (syncio | fua)					\
	...
	if (rw && if_throttle_start_size > 0 && total_len / 1024 >= if_throttle_start_size) {
		xio_limit_sleep(&if_throttle, total_len / 1024);
	}

	// TODO: implement readahead in the infrastructure
	if (ahead) {
...
#error FIXME: the current infrastructure cannot deal with HIGHMEM / kmap()
		...
		unlock:
			...
			if (!aio) {
				 ...
				 */
				aio->io_flags = (AIO_PERF_NOMETA | AIO_PERF_NOSYNC);
				if (i + 1 >= bio->bi_vcnt) {
					_SET_AIO_FLAGS(aio);
				}
