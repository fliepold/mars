...
bool phase0_startio(struct trans_logger_aio_aspect *orig_aio_a)
{
	 ...
	 * may drop one refcount on orig_aio if the original caller
	 ...
	 * absolutely the last action on the orig_aio, since it is only executed
...
static
struct rank_info extra_rank_aio_flying_log[] = {
	{     0,    0 },
	{     1,   10 },
	{    16,   30 },
	{    31,    0 },
...
static
struct rank_info extra_float_rank_aio_flying_io[] = {
	{     0,    0 },
	{    31,    0 },
	{    32, -200 },
	{ RKI_DUMMY }
...
static
struct rank_info extra_nofloat_rank_aio_flying_io[] = {
	{     0,    0 },
	{    63,    0 },
	{    64, -200 },
	{ RKI_DUMMY }
...
static
struct rank_info *extra_rank_aio_flying[2][LOGGER_QUEUES] = {
	[0] = {
		[0] = extra_rank_aio_flying_log,
		[1] = extra_float_rank_aio_flying_io,
		[2] = extra_float_rank_aio_flying_io,
		[3] = extra_float_rank_aio_flying_io,
	},
	[1] = {
		[0] = extra_rank_aio_flying_log,
		[1] = extra_nofloat_rank_aio_flying_io,
		[2] = extra_nofloat_rank_aio_flying_io,
		[3] = extra_nofloat_rank_aio_flying_io,
	},
};
...
int _do_ranking(struct trans_logger_brick *brick, struct rank_data rkd[])
{
	...
	for (i = 0; i < LOGGER_QUEUES; i++) {
		...
		if (extra_rank_aio_flying[floating_mode][i]) {
			ranking_compute(&rkd[i], extra_rank_aio_flying[floating_mode][i], aio_flying);
		}
...
void _exit_inputs(struct trans_logger_brick *brick, bool force)
{
	...
	for (i = TL_INPUT_LOG1; i <= TL_INPUT_LOG2; i++) {
		...
		if (input->is_operating &&
			...
			XIO_DBG("cleaning up input %d (log = %d old = %d), old_applying = %d old_logging = %d\n", i, brick->log_input_nr, brick->old_input_nr, old_applying, old_logging);
			exit_logst(logst);
...
void trans_logger_replay(struct trans_logger_brick *brick)
{
	...
	for (;;) {
		...
		if (lh.l_code != CODE_WRITE_NEW) {
			...
			XIO_RPL("apply %lld %lld (pos=%lld status=%d)\n", finished_pos, new_finished_pos, lh.l_pos, status);
			if (unlikely(status < 0)) {
				brick->replay_code = status;
				XIO_ERR("cannot apply data at pos = %lld len = %d, status = %d\n", lh.l_pos, len, status);
				break;
			} else {
				finished_pos = new_finished_pos;
			}
	...
	if (status >= 0 && finished_pos == brick->replay_end_pos) {
		...
		XIO_INF("TOLERANCE: logfile is DAMAGED at %lld (of %lld)\n", finished_pos, brick->replay_end_pos);
		brick->replay_code = status;
	} else {
...
void __exit exit_xio_trans_logger(void)
{
